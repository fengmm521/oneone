...
#汉化字体
def hanhuaFont(v):
    cmd = '/bin/cp -f oneonesource/zh/mac_zh/code/minorGems/game/Font.cpp v%d/minorGems/game/Font.cpp'%(v)
    os.system(cmd)
    cmd = '/bin/cp -f oneonesource/zh/mac_zh/code/minorGems/game/Font.h v%d/minorGems/game/Font.h'%(v)
    os.system(cmd)
def hanhuaMapCPP(ver):
    hmapcpppth = 'oneonesource/zh/mac_zh/code/OneLife/server/map.cpp'
    tcpppth = 'v%d/OneLife/server/map.cpp'%(ver)
    f = open(tcpppth,'r')
    lines = f.readlines()
    f.close()
    outstr = ''
    for i,v in enumerate(lines):
        if v[0] == '#' and v.find('#include <values.h>') != -1:
            v = '''//#include <values.h>
#include <limits.h>
'''      
        outstr += v
...
def hanhuaLivingLifePageCpp(ver):
    tcpppth = 'v%d/OneLife/gameSource/LivingLifePage.cpp'%(ver)
    f = open(tcpppth,'r')
    lines = f.readlines()
    f.close()
    outstr = ''
    for i,v in enumerate(lines):
        if v.find('static float pencilErasedFontExtraFade =') != -1:
            v = 'static float pencilErasedFontExtraFade = 0.15;\n'
        outstr += v
    f = open(tcpppth,'w')
    f.write(outstr)
    f.close()
