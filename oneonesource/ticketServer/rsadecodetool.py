...
class prpcrypt():
    ...
    def saveGhostKeyToFile(self):
        ...
        if sys.version_info > (3,0):
            ...
            der = self.gprivate_pem._save_pkcs1_der()
            f.write(rsa.pem.save_pem(der, 'RSA PRIVATE KEY').decode())
        f.close()
        ...
        if sys.version_info > (3,0):
            ...
            der = self.gpublic_pem._save_pkcs1_der()
            f.write(rsa.pem.save_pem(der, 'PUBLIC KEY').decode())
            # f.write(self.gpublic_pem.save_pem(,'PUBLIC KEY'))
        f.close()
    ...
    def readGhostKeyFromFile(self):
        ...
        if os.path.exists(pripth) and os.path.exists(pubpth):
            ...
            if sys.version_info > (3,0):
                ...
                der = rsa.pem.load_pem(f.read(), 'PUBLIC KEY')
            f.close()
        else:
