...
#ifdef _WIN32
   //define something for Windows (32-bit and 64-bit, this part is common)
    wchar_t* AnsiToUnicode(const char* lpcstr)   //参数lpcstr类型也可是char*  
    {   
        wchar_t* Pwstr;  
        int  i;  
...
//define something for Windows (32-bit and 64-bit, this part is common)
...
#else
   //define something for lunux or mac (32-bit only)
   glEnable ( GL_TEXTURE_2D );  
   ...
   // std::string dstr = wstring_to_utf8(wstr);
   // char buff[256];
   //  snprintf(buff, sizeof(buff), "/bin/echo \"%s\"", "aaabbbccc");
   //  std::string buffstr = buff;
   //  system(buffstr.c_str());
   // system("/bin/echo \"8888\"");
   //drawText(wstr.c_str(), inPosition.x, inPosition.y, 2048, mScaleFactor, inAlign); 
   // system("/bin/echo \"99999\""); 
/*
